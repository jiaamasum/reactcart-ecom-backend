...
```
- Note: `slug` is URL-safe; use `id` as the identifier.
...
curl -s -X PATCH -H "Authorization: Bearer <ADMIN_TOKEN>" \
  -H "Content-Type: application/json" \
  -d '{ "price": 24.99, "discountedPrice": 19.99 }' \
  http://localhost:8080/api/admin/products/prod-1
```
...
curl -s -X PATCH -H "Authorization: Bearer <ADMIN_TOKEN>" \
  -H "Content-Type: application/json" \
  -d '{ "name": "Audio" }' \
  http://localhost:8080/api/admin/categories/cat-1
```
